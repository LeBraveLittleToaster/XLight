...
public class MtsLightStateValidator {
  ...
  public static Mono<MtsLightState> validateInsertLightState(MtsMode mtsMode,
      List<MtsValue> values) {

    if (!isModeAndValueIndexesEqual(mtsMode, values)) {
      return Mono.error(
          new IndexMissmatchException("MtsValue ids and mode input ids don´t match!"));
    }
    return Mono.just(
  ...
  private static boolean isModeAndValueIndexesEqual(MtsMode mtsMode, List<MtsValue> values) {
    List<Long> valuesIds = values.stream()
        .map(MtsValue::getValueId)
